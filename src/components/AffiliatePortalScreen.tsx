...
export const AffiliatePortalScreen: React.FC<AffiliatePortalScreenProps> = ({ onNavigate }) => {
  ...
  const officialLink = data?.affiliate?.code 
    ? (typeof window !== 'undefined' && window.location.origin 
        ? `${window.location.origin}/afiliado/${data.affiliate.code}` 
        ...
        <div className="max-w-md w-full bg-[#0a1f3d] border border-white/10 rounded-2xl p-8 text-center shadow-2xl">
          ...
          <div className="space-y-3">
            <button
              onClick={async () => {
                try {
                  ...
                  alert(e.message || 'Erro ao fazer login com Google.');
                }
              }}
              className="w-full py-3 bg-white hover:bg-slate-100 text-slate-900 font-bold rounded-xl transition-all cursor-pointer text-sm shadow-lg flex items-center justify-center gap-2"
            >
            ...
            <button
              ...
              className="w-full py-2.5 bg-purple-600 hover:bg-purple-500 text-white font-semibold rounded-xl transition-colors cursor-pointer text-xs"
            >
              Fazer Login com E-mail / Senha
            </button>
            <button
              onClick={() => { window.location.href = '/'; }}
              className="w-full py-2 bg-white/5 hover:bg-white/10 text-slate-300 font-medium rounded-xl transition-colors cursor-pointer text-xs"
            >
