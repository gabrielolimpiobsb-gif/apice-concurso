...
export interface PdfChapter {
  ...
  pdfUrl?: string; // specific chapter pdf if split, or inherits from course
}
...
export interface PdfCourse {
  ...
  title: string;
  subtitle?: string;
  discipline: string; // Ex: Direito Constitucional, Língua Portuguesa, etc.
  career: string; // Ex: Policial, Fiscal, Tribunais, Geral
  targetExam?: string; // Ex: PRF, PF, Receita Federal, TJDFT
  author: {
    name: string;
    title?: string;
    avatarUrl?: string;
  };
  coverGradient: string; // Tailwind gradient
  coverAccentColor?: string;
  totalPages: number;
  totalModules: number;
  fileSizeMb?: number;
  pdfUrl: string; // The URL to the PDF file (hosted on Firebase Storage, Google Drive, AWS, or CDN)
  downloadUrl?: string;
  description: string;
  highlights: string[];
  tags: string[];
...
export interface UserPdfProgress {
  courseId: string;
  currentPage: number;
  totalPages: number;
